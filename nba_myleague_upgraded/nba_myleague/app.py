...
# UPGRADE: Named divisions (3 per conference, 5 teams each -- matches real
# league structure) were never actually part of the team data model before;
# only conference (East/West) existed. Split each conference's 15 teams into
# three named divisions in the same order they're already listed.
TEAM_DIVISION = {}
for _conf_teams, _div_names in [(EAST_TEAMS, ["Founders", "Liberty", "Metro"]),
                                  (WEST_TEAMS, ["Pacific", "Summit", "Coastal"])]:
    for _i, _dname in enumerate(_div_names):
        for _t in _conf_teams[_i * 5:(_i + 1) * 5]:
            TEAM_DIVISION[_t] = _dname
...
SIM_STATE = {
    ...
    # UPGRADE: era concept disabled per request -- the game now always boots
    # straight into the Modern era (already the default "era" value below)
    # instead of showing an era-picker screen first. era_chosen starts True
    # so the frontend's era-picker gate (`if (!state.era_chosen)`) never
    # triggers; onboarding now goes straight to team selection.
    "era_chosen": True,
    "year": START_YEAR,
...
def load_league_from_data_files():
    """UPGRADE: makes the 6 division JSON files (data/division_*.json) the
    actual source of truth for teams/rosters, instead of pure procedural
    generation -- hand-edit a player's attributes/tendencies/contract/etc.
    in these files and the game loads exactly that on next boot.

    All-or-nothing by design: if EVERY division file loads and parses
    cleanly, the whole league comes from the files. If even one is
    missing, malformed, or fails to parse, this aborts entirely and
    seed_league() falls back to its original full procedural generation --
    a partial mix of file-sourced and randomly-generated teams would be a
    much worse failure mode (inconsistent rosters, broken draft-pick
    ownership, etc.) than just regenerating everything from scratch.
    Returns True if the file-based league was loaded successfully.
    """
    data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
    division_files = ["division_founders.json", "division_liberty.json", "division_metro.json",
                       "division_pacific.json", "division_summit.json", "division_coastal.json"]
    loaded_teams = {}
    loaded_players = {}
    try:
        for fname in division_files:
            ...
            if not teams or not players:
                raise ValueError(f"{fname} is missing teams or players data")
            for tname, tdata in teams.items():
                if tname not in NBA_TEAMS:
                    raise ValueError(f"{fname} has unknown team '{tname}' -- not in NBA_TEAMS")
                loaded_teams[tname] = tdata
            for pname, pdata in players.items():
                loaded_players[pname] = pdata
        if set(loaded_teams.keys()) != set(NBA_TEAMS):
            missing = set(NBA_TEAMS) - set(loaded_teams.keys())
            raise ValueError(f"division files don't cover all 30 teams -- missing: {missing}")
    except Exception as e:
        print(f"[data-file league load] Aborting file-based load, falling back to procedural generation. Reason: {e}")
        return False

    # All 6 files loaded cleanly -- commit to using them.
    SIM_STATE["teams"].update(loaded_teams)
    SIM_STATE["players"].update(loaded_players)
    _NAME_REGISTRY.update(loaded_players.keys())

    # Free agents and draft picks are lower-stakes -- load them if present
    # and valid, but don't abort the whole league load over just these.
    try:
        with open(os.path.join(data_dir, "free_agents.json"), "r") as f:
            fa_payload = json.load(f)
        fa_dict = fa_payload.get("free_agents", {})
        SIM_STATE["free_agents"] = list(fa_dict.values())
        for pname, pdata in fa_dict.items():
            SIM_STATE["players"][pname] = pdata
            _NAME_REGISTRY.add(pname)
    except Exception as e:
        print(f"[data-file league load] free_agents.json not loaded ({e}) -- generating procedurally instead.")
    ...
    try:
        with open(os.path.join(data_dir, "draft_picks.json"), "r") as f:
            dp_payload = json.load(f)
        SIM_STATE["draft_picks"].update(dp_payload.get("draft_picks", {}))
    except Exception as e:
        print(f"[data-file league load] draft_picks.json not loaded ({e}) -- generating procedurally instead.")
        for team_name in NBA_TEAMS:
    ...
    print(f"[data-file league load] Loaded {len(loaded_teams)} teams, {len(loaded_players)} rostered players from data/ files.")
    return True
...
def seed_league():
    ...
    # UPGRADE: try the data/ JSON files first (see load_league_from_data_files
    # docstring) -- only fall through to full procedural generation below if
    # that fails for any reason.
    if load_league_from_data_files():
    ...
    for team_name in NBA_TEAMS:
        SIM_STATE["teams"][team_name] = {
            ...
            "division": TEAM_DIVISION.get(team_name),
...
# UPGRADE: on-demand session reset ("tab refresh restarts everything unless
# you explicitly save"). Snapshot the exact state right after the app's
# normal boot sequence finishes (fresh 30-team league, era already Modern,
# team not yet chosen, full 1984-> backstory already generated) so a reset
# is byte-for-byte identical to a genuine fresh server start -- not a
# hand-rolled subset of fields that could drift out of sync with the real
# bootstrap sequence over time.
import copy as _copy
PRISTINE_BOOT_STATE = _copy.deepcopy(SIM_STATE)


@app.route('/api/reset_new_session', methods=['POST'])
def api_reset_new_session():
    """UPGRADE: previously the ONLY way to get a truly fresh start was a
    full server restart -- the game auto-persisted in memory, so simming a
    few days and then refreshing the tab just silently resumed exactly
    where you left off, with no way to discard unsaved progress short of
    explicitly saving over it. The frontend now calls this once,
    automatically, on every real page load/refresh (not on in-app tab
    switches) -- so refreshing always starts a clean new onboarding flow
    unless you explicitly used Load Game to restore a save slot instead."""
    global SIM_STATE
    SIM_STATE.clear()
    SIM_STATE.update(_copy.deepcopy(PRISTINE_BOOT_STATE))
    return jsonify({"success": True})


@app.route('/api/reload_data_files', methods=['POST'])
def api_reload_data_files():
    """UPGRADE: hand-editing data/division_*.json and then needing to fully
    stop and restart the whole `python3 app.py` process to see the change
    (since seed_league() -- and the file reads inside it -- only ever runs
    once, at server startup) is a lot of friction for something meant to
    be iterated on. This re-reads every data file from disk RIGHT NOW,
    rebuilds the league from them, and updates PRISTINE_BOOT_STATE too so
    a plain tab refresh reflects the edit from now on as well, instead of
    reverting to the stale snapshot taken at original server boot.
    Same all-or-nothing safety as the original load: a broken/invalid file
    aborts the reload entirely and leaves your currently-running league
    completely untouched, rather than risking a corrupted in-progress game.
    """
    global SIM_STATE, PRISTINE_BOOT_STATE
    backup = _copy.deepcopy(SIM_STATE)
    SIM_STATE["teams"] = {}
    SIM_STATE["players"] = {}
    SIM_STATE["free_agents"] = []
    SIM_STATE["draft_picks"] = {}
    ok = load_league_from_data_files()
    if not ok:
        SIM_STATE.clear()
        SIM_STATE.update(backup)
        return jsonify({"success": False, "reason": "One or more data files failed to load (bad JSON, missing team, etc). Nothing was changed -- check the server console log for the exact reason."})
    SIM_STATE["draft_class"] = generate_draft_class(SIM_STATE["year"])
    PRISTINE_BOOT_STATE = _copy.deepcopy(SIM_STATE)
    return jsonify({"success": True, "teams": len(SIM_STATE["teams"]), "players": len(SIM_STATE["players"])})
